...
public actor Qwen3ASRSTT {
    ...
    /// RTranslate: keep the allocator's reusable cache bounded while this backend is active.
    public static func configureMemoryBudget() {
        MLX.Memory.cacheLimit = 128 * 1024 * 1024
    }

    /// RTranslate: called only after all model/inference owners have drained.
    ...
    /// Reclaim per-utterance temporary buffers after the inference function has returned.
    public static func trimMemoryPool() {
        Stream.gpu.synchronize()
        MLX.Memory.clearCache()
    }

    /// Explicit diagnostics only; querying these values initializes the MLX runtime.
    public static func memoryUsage() -> (active: Int, cache: Int, peak: Int) {
        let value = MLX.Memory.snapshot()
        return (value.activeMemory, value.cacheMemory, value.peakMemory)
    }
